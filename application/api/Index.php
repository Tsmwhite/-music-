...
// +----------------------------------------------------------------------
// | 版权所有 2015~2018 长春市竹子科技有限公司 [ http://www.cczhuzi.cn ]
// +----------------------------------------------------------------------
// | 开发团队 PHP技术团队
// +----------------------------------------------------------------------
...
// AlibabaCloud::accessKeyClient('LTAIZ2OXDuYTWcC3', 'oDopaAu0APjee1NcoEnO3lyda9FkMb')->asDefaultClient();
 ...
 * @author 张一然 <mydopamine@163.com>
 * @date 2018/02/25
 */
class Index extends Controller
{ 
	//商家配送配送费价格
    public function carry_money()
    {
	    $result = array(
	                "code" => 200,
	                "message" => "请求成功",
	                "money" => 5,//配送费
	                "address" => '449 WATERDALE ROAD'
	            );
    ...
    public function banner()
    {
       ...
       * banner
       */
        $banner_list = Db::name("ShopPic")->where(['is_deleted' => '0'])->select();
    ...
    public function index()
    {
         ...
         * 城市列表加缓存
         */
        if(!empty(Cache::get('citylist'))){
            $list = Cache::get('citylist');
        }else{
            $py = new py_class();

            $list = Db::name("ShopRegion")->where(['type' => '2'])->select();

            foreach ($list as $key => $val) {
                $letter = $py->str2py($val['name']);
                $list[$key]['first'] = $letter;
            }
            array_multisort(array_column($list,'first'),SORT_ASC,$list);

            $list = $py->array_group_by($list,'first');
            Cache::set('citylist',$list);
         ...
         */
        $total_recharge = 0;
        $list = Db::name("shop_order")->where(['openid'=>$openid,'flag' => '2','status'=>'2'])->select();
        foreach ($list as $key => $val) {
            if($val['hasbonus'] > 0 ){
                $recharge = Db::name("shop_recharge")->where(['id'=>$val['hasbonus']])->find();
                $total_recharge += $recharge['money']+$recharge['song'];
            }else{
                $total_recharge += $val['price'];
            }
        }
         ...
         * 专业洗护分类
         */
        $category_list = Db::name("ShopCategory")->where(['status' => '1','pid'=>0])->field('id,title,pic')->order('sort asc')->limit('4')->select();
       
        $this->assign('category_list',$category_list);
        $this->assign('change','index');
        return $this->fetch();
    }
    /**
     * 我的
    ...
    public function member(){
        $this->assign('change','member');
        $openid = $this->oAuth(true);
        
        $info = Db::name("ShopMember")->where(['openid'=>$openid,'is_deleted' => '0'])->find();
        if($info){
            /**
             * 查询优惠券总数
             */
            $total = Db::name("shop_coupon_exchange")->where(['mid'=>$info['id'],'is_deleted' => '0'])->count();
             ...
             * 统计积分
             */
            $list_integral = Db::name("shop_integral")->where(['openid'=>$openid])->select();
            $integral = 0;
            foreach ($list_integral as $key => $v) {
            	if($v['flag'] == 3){
					$integral-=$v['integral'];
            	}else{
					$integral+=$v['integral'];
        ...
        $result = AlibabaCloud::rpcRequest()
                          ...
                          ->options([
                                        'query' => [
                                            ...
                                            // "From" => "1234567890",
                                            "Message" => "【Snaker Clean】 Your code is".input('code'),
                                        ],
                                    ])
                          ->request();
    ...
    public function resign(){
       ...
       $info = Db::name("ShopMember")->where(['phone'=>$mobile,'is_deleted' => '0'])->find();
            ...
            $result = array(
             ...
             exit;
            	...
            	$arr = array(
            		...
            		'phone' => $mobile,
            		'sharecode' => input('sharecode'),
            		'recommend' => DataService::createSequence(6, 'invitationcode')
       				);

            		$result = Db::name("ShopMember")->insert($arr);
            		if($result) {

		            /**
		             * sharecode不空，为会员创建5元优惠券
		             */
		            $mid = Db::name("ShopMember")->getLastInsID();
		            if(input('sharecode')){
		                $coupon_data = array(
		                    'code' =>DataService::createSequence(6, 'COUPON'),
		                    'title' =>'5元优惠券',
		                    'full_money' =>1,
		                    'deduction' => 5,
		                    'has_exchange' =>1
		                );
		                Db::name("shop_coupon")->insert($coupon_data);
		                $cid = Db::name("shop_coupon")->getLastInsID();
		                $coupon_exchange_data = array(
		                    'coupon_id'=>$cid,
		                    'mid' =>$mid,
		                    'addtime' =>time(0)
		                );
		                Db::name("shop_coupon_exchange")->insert($coupon_exchange_data);
		            }
    ...
    public function sendMsg($mobile,$code,$type_id){
        ...
        $accessKeyId = 'LTAIetgDZSmK8z8f';  //阿里云短信获取的accessKeyId
        $accessKeySecret = 'S896EhQ9sxUXD4xrFiATqkfcUdvYz8';    //阿里云短信获取的accessKeySecret
        //这个个是审核过的模板内容中的变量赋值，记住数组中字符串code要和模板内容中的保持一致
        //比如我们模板中的内容为：你的验证码为：${code}，该验证码5分钟内有效，请勿泄漏！
        $templateParam = array("code"=>$code);           //模板变量替换
        $signName = '麦鱼'; //这个是短信签名，要审核通过
        if($type_id==1){
            $templateCode = 'SMS_149350350';   //登录
        }else if($type_id==2){
            $templateCode = 'SMS_149350348';   //注册
        }else{
            $templateCode = 'SMS_149350347';   //改密
        }
   ...
   /**
    ...
    public function forgot() {
       ...
       $password = input('password');
        $info = Db::name("ShopMember")->where(['phone'=>$mobile,'is_deleted' => '0'])->find();
        if ($info) {
           $change = Db::name('ShopMember')->where(['phone'=>$mobile,'is_deleted' => '0'])->update(['password' => $password]);
           if ($change) {
            $result = array(
                        "code" => 200,
                        "message" => "修改密码成功",
...
//     const API_ACCOUNT= 'N2256174'; // 创蓝API账号
    
//     const API_PASSWORD= 'yYSl1h3X4Q2fd6';// 创蓝API密码
...
class py_class{
     ...
     */
    function str2py($s){
        $s=preg_replace("/\s/is","_",$s);//空格 回车等替换成'_'
        ...
        // return strtolower($py);//转换为小写
        return strtoupper($py);//转换为小写
    }
    ...
    function str2pys($s){
        $s=preg_replace("/\s/is","_",$s);//空格 回车等替换成'_'
        $s=preg_replace("/(|\~|\_|\`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\-|\+|\=|\{|\}|\[|\]|\||\\|\:|\;|\"|\'|\<|\,|\>|\.|\?|\/)/is","",$s);//斜线下划线等替换成空
        $py="";
        $i=0;
        ...
        for($i=0;$i<strlen($s);$i++){
            ...
            if(strlen($py)>=20)return $py;//最大长度为20
        }
        return strtolower($py);//转换为小写
    }
